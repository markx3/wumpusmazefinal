...
int play(Player *player, Wumpus *wumpus, char **maze, PilhaGenerica *pilha, int endX, int endY, int lin, int col) {
	...
	while(player->x != endX &&
		...
		if(maze[next.x][next.y] == 'F') printf("> Player: What is that disgusting smell?\n");
		...
		printf("Wumpus Maze v1.0b         SCORE: %d\n", player->score);
	...
	system("clear");
	print_maze(maze, lin, col);
	printf("> Player: Finally I got out of this scary maze!\n");
...
int check_trap(char **maze, int i, int j) {
	...
	for (count = -1; count <= 1; count++) {
		if (maze[i+1][j+count] == 'P') printf("> Player: What a chilly breeze.\n");
	}
	for (count = -1; count <= 1; count++) {
		if (maze[i-1][j+count] == 'P') printf("> Player: What a chilly breeze.\n");
	}
	if (maze[i][j-1] == 'P') printf("> Player: What a chilly breeze.\n");
	if (maze[i][j+1] == 'P') printf("> Player: What a chilly breeze.\n");
	return 1;
}
...
int pick_gold(Player *player) {
	...
	printf("> Player: My luck is changing. This gold will be useful to kill that hideous Wumpus.\n");
	return TRUE;
}
...
int shoot_arrow(Wumpus *wumpus, Player *player) {
	...
	printf("> Wumpus: AAAAAAWWW AWWWW NOOO I'M DEAD NOW WOOOOOOT\n");
	return TRUE;
}
...
int print_maze(char **maze, int lin, int col) {
	int i = 0, j = 0;
	for (i = 0; i <= lin; i++) {
		for (j = 0; j <= col; j++) {
			printf("%c", maze[i][j]);
